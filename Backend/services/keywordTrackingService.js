...
export async function keyworTracking(tracking){
    try{
        let result;
        ...
        if(result.success){
            ...
            //Update stats
            tracking.position.Change = prev && result.data.position ? prev - result.data.position :0;
            ...
            const idx =tracking.rankHistory.findInx((h)=>h.date.toDateString() === today.toDateString());
